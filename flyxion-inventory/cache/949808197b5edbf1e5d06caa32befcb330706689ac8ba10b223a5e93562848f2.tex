...
close constraints. Signal production and constraint closure are therefore
decoupled at equilibrium.
\end{proposition}
...
downstream systems evolve. The operative notion is therefore not
...
Each of the three terms requires precise definition to resist strategic
behavior. The following three sections address them in turn.
...
Dependency is not declared; it is measured by failure propagation. A
...
graph (DAG) of contributing actions, and assigns credit via marginal
contribution.
...
little, regardless of how long it nominally persists.
...
cumulative residue of closures along it:
\[
  V(\gamma) = \sum_i \rho_i
\]
This replaces pointwise metrics with path-dependent evaluation.
...
The equilibrium under $\mathcal{S}'$ differs from the equilibrium under
$\mathcal{S}$ in the following respects:
\begin{itemize}
  \item Activity without closure is not rewarded.
  \item Mediation without constraint interaction is not rewarded.
  \item Credential signaling disconnected from residue production is not
    rewarded.
  \item Persistence under stress and causal integration are rewarded.
\end{itemize}
...
Without economic routing, the residue framework becomes a descriptive
overlay---a better dashboard for the same managerial layer it is
intended to replace. The mechanism must place resource allocation
decisions downstream of $\rho(\gamma)$, bypassing institutional
hierarchy.
...
The validation criterion for a first implementation is modest: given
two contributors with similar activity levels, does the residue-based
metric separate them in a way that aligns with expert judgment? If that
holds even weakly, the architecture becomes economically interesting and
the selection-function argument moves from structural to demonstrable.
...
dissolve, three things follow. First, the evaluation vocabulary
atrophies, because it no longer maps to anything observable. Second,
agents lose access to a shared coordinate system over which trajectories
can be compared. Third, without observable residue, evaluation collapses
back onto visible signals---appearance, affiliation, stated identity.

This is not primarily a cultural or emotional phenomenon. It is a
measurement collapse: the system can no longer distinguish trajectories
that differ in constraint closure, because the constraint surface is no
longer exposed.
...
internal solution. The dominant response is not to interact more deeply
with constraints---that path is closed if the constraint surface is not
exposed---but to revalue the ranking system itself.

Formally, if an agent perceives that $\rho$ is high for others but
inaccessible to them, and that $\sigma$ is the operative selection
criterion, the rational move is:
\[
  \text{invert evaluation criteria}
  \;\Rightarrow\;
  \text{treat high-}\rho\text{ signals as illegitimate}
  \;\Rightarrow\;
  \text{optimize for low-order signals}
\]

This is the structural mechanism behind what is sometimes described as
resentment or transvaluation: not primarily an emotional state, but a
rational adaptation to operating under $\mathcal{S}$ without access to
$\mathcal{S}'$.
...
spaces. A role embedded a fixed constraint set, provided repeated
exposure to that constraint set, and made success and failure under
stress observable to others. The role was not merely a job
description---it was a scaffold for accumulating residue in a shared
measurement system. Evaluation vocabulary was calibrated against that
accumulation.

When roles dissolve into self-defined identity, the constraint surface
does not automatically transfer. Agents are left in unconstrained space
with no stable external basis for evaluation. The vocabulary that once
described trajectory differences becomes decorative rather than
functional, because the underlying structures it referred to are no
longer exposed.
