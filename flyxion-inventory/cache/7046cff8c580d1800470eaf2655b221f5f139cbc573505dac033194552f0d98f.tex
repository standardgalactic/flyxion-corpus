...
$\text{km} = (\text{km/h})\cdot\text{h}$. \checkmark
...
\]
She then tests: $3(2)+2=8$. \checkmark\ Her causal model is confirmed.
\end{myexample}
...
\[
  \frac{a}{b}=\frac{c}{d}.
\]
Cross-multiplying yields $ad=bc$, a linear equation.
...
\]
Doubling the radius to $r=4$ km multiplies area by $2^2=4$, quadrupling the
capacity. Scaling is not linear.
\end{myexample}
...
\[
  ...
  \end{cases}
\]
where $L$ is labour, $F$ food supply, and $E$ energy production. With $L=100$:
$F=200$, $E=300$. If a drought reduces $L$ to 80, then $F=160$ and $E=240$.
The constraint cascade propagates through the entire system.
\end{myexample}
...
\begin{tikzpicture}[scale=1.1]
  ...
  \end{scope}
  % Arrow
  \draw[arrow] (1.3,0.5)--(3.2,0.5) node[midway,above,font=\scriptsize]{$A=$\scriptsize$\begin{pmatrix}1&1\\0&1\end{pmatrix}$};
\end{tikzpicture}
\end{center}
...
\]
Conservation is the condition $N\gamma=\mathbf{0}$.
\end{myexample}
...
\[
  a^2+b^2=c^2.
\]
This is the fundamental constraint on spatial dimensions: the hypotenuse is
completely determined by the two legs.
\end{theorem}
...
\]
\textit{Dimensional check:} $\text{W/m}^2=\text{W}/\text{m}^2$. \checkmark
...
\]
This inverse-square law is why apparent brightness falls off with the square of
distance --- a constraint on spatial geometry.
\end{myexample}
...
\]
Population capacity:
$400\times5.4\pi\times100\approx679{,}000$.
...
\]
For a right triangle with acute angle $\theta$, adjacent side $a$, opposite
...
\[
  \sin^2\theta+\cos^2\theta=1.
\]
This expresses the conservation of magnitude under rotation: the squared
components of a unit vector always sum to one.
\end{theorem}
...
\]
where $t$ is in days. Period $=5$ days, amplitude $=2$, mean brightness
$=10$.
...
\]
The oscillation period encodes the star's intrinsic luminosity, allowing
...
\]
When $\delta=0$, amplitude doubles (constructive interference). When
$\delta=\pi$, amplitude vanishes (destructive interference). The constraint
...
\]
The $x$- and $y$-projections are sinusoidal functions of time --- this is
...
\begin{tikzpicture}[scale=1.2]
  ...
  \draw[varrow] (2.2,0)--(4.5,0) node[right,font=\scriptsize]{$t$};
  \begin{scope}[xshift=3.8cm]
  \draw[ctBlue,thick,smooth,samples=60,domain=0:360]
      plot ({0.6*\x/360},{sin(\x)});
  \node[font=\scriptsize,ctBlue,below] at (0.6,-1.1){$x(t)=\cos\omega t$};
  \end{scope}
\end{tikzpicture}
\end{center}
...
\[
  n = \frac{m}{M}
\]
is the chemical analogue of the proportionality $y=kx$.
\end{mydefinition}
...
\]
\textit{Dimensional check:} $\text{g}/(\text{g/mol})=\text{mol}$. \checkmark
\end{myexample}
...
\]
\textit{Verification row by row:} C: $4=4$; H: $12=12$; O: $14=14$. \checkmark
\end{myexample}
...
could produce $5\times\tfrac23\approx3.33\;\text{mol NH}_3$.
...
Starting with 4 mol each of $A,B,D,F$ and 6 mol of nothing (i.e., $C$ and
$E$ are intermediates), determine the theoretical yield of $G$. Identify
which reagent is limiting at each stage.
\end{challenge}
\end{exercises}
...
\]
which expresses conservation of a density $\rho$ flowing with velocity
$\mathbf{v}$. Algebra, stoichiometry, and field theory all encode the same
structural idea: \emph{what flows in must flow out}.
\end{myexample}

\begin{deepnote}
Appendix~D formalises the stoichiometric matrix framework, and Appendix~E
derives the RSVP field equations as the continuum limit of this conservation
structure.
\end{deepnote}
...
$F=2L$, $E=F+L$
from Chapter~4 is a two-node constraint network: one node for each resource,
one edge for each relation. Le Guin's utopian societies must maintain exactly
such networks at equilibrium \citep{LeGuin1974}.
\end{myexample}
...
\]
The \textbf{solution locus} is
$\mathcal{C}=\{x\in\R^n\mid F_\alpha(x)=0\;\forall\alpha\}$.
...
\]
Exact solutions satisfy $\mathcal{R}=0$; approximate solutions minimise
$\mathcal{R}$. This is the \textbf{least-squares principle}, which is also
the precursor to entropic relaxation in the RSVP framework (Appendix~E).
...
\]
An $L^2$-type inner product extends the Euclidean structure:
\[
  \langle X_1,X_2\rangle=
  \int_\Omega\!\left(\Phi_1\Phi_2+\mathbf{v}_1\cdot\mathbf{v}_2+S_1S_2\right)dx.
\]
Geometric intuition (orthogonality, distance, angle) persists verbatim in this
...
\]
where $\mathbf{r}(\mathbf{c})$ is the reaction rate vector. This is the
\emph{Chemical Master Equation} in its deterministic (mass-action) limit.
...
\[
  ...
  S(x,t)\in\R,
\]
representing plenum density (scalar organisation), vector flow, and entropy
...
\[
  ...
  \right]dx.
\]
In dissipative regimes the dynamics approximate gradient descent on
$\mathcal{F}$:
...
\]
recovering a Hubble-like proportionality as an effective large-scale integral
law --- without assuming metric expansion of space.
...
\]
where structured noise decomposes as
$\xi=\xi_\text{white}+\xi_\text{structured}$,
...
\]
where $L_\text{task}$ is task-predictive loss and $C(m)$ is representational
complexity.
